...
public final class WorldClient implements AutoCloseable {
    ...
    /**
     * Returns the latest authoritative inventory update received from the server.
     *
     * @return the latest inventory update, if present
     */
    ...
    /**
     * Requests pickup of the nearest loot item.
     *
     * @param playerEntityId the local player entity id
     * @throws IOException if sending fails
     */
    public void sendPickupLoot(SharedEntityId playerEntityId) throws IOException {
        ServerConnection connection = activeConnection;
        if (connection == null || !connection.isOpen()) {
            return;
        }
    ...
    /**
     * Requests equipping an item from the given inventory slot.
     *
     * @param playerEntityId the local player entity id
     * @param inventorySlotIndex zero-based inventory slot index
     * @throws IOException if sending fails
     */
    public void sendEquipItem(SharedEntityId playerEntityId, int inventorySlotIndex) throws IOException {
        ServerConnection connection = activeConnection;
        if (connection == null || !connection.isOpen()) {
            return;
        }
        connection.sendLine(packetCodec.encode(new EquipItemPacket(playerEntityId, inventorySlotIndex)));
    }
        ...
        readerThread = Thread.ofVirtual().name("world-client-reader").start(() -> {
            try {
                for (String line = connection.readLine(); line != null; line = connection.readLine()) {
                    Packet packet = packetCodec.decode(line);
